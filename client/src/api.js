...
const OFFLINE_MESSAGE =
  'Cannot reach the server. The API may not be running — make sure `npm run dev` is active in the project folder.'
...
export async function api(path, { method = 'GET', body } = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      res.status >= 500 ? OFFLINE_MESSAGE : `Something went wrong (HTTP ${res.status}).`
    )
  }
  return data ?? {}
}
